...
function toggleSetsTable() {
  showSetsTable.value = !showSetsTable.value
  // Maintain focus on input after toggle
  nextTick(() => focusInput())
}

// Watch for changes in active questions and validate current question
watch(
  () => topicsStore.getActiveQuestions(),
  () => {
...
<template>
  <div class="w-full flex justify-center">
    ...
    <h1 class="text-slate-700 text-3xl font-bold mb-5">Learn Like K</h1>
    ...
    <div v-else class="flex-1 flex flex-col">
      ...
      <div class="mt-4 min-h-96">
        <div v-if="topicsStore.getActiveQuestions().length === 0" class="text-center">
          <div class="text-2xl text-gray-500 p-7 bg-gray-50 rounded-xl border-2 border-gray-200 mb-6">
            No questions selected
          </div>
          <p class="text-gray-600 mb-4">Choose some questions below to start practicing</p>
        </div>
      ...
      <div class="text-center mt-8 mb-4">
        <button 
          @click="toggleSetsTable"
          class="text-gray-500 hover:text-gray-700 text-sm underline cursor-pointer bg-transparent border-none"
        >
          {{ showSetsTable ? 'hide' : 'choose questions' }}
        </button>
      </div>
      
      <QuestionSetsTable v-if="showSetsTable" class="mb-6" />
    </div>
